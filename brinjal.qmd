...
beans_dambulla <- veg_filled %>%
  filter(Item == "Brinjal", Type == "Retail", Market == "Dambulla") %>%
  select(Date, Price) %>%
...
# Draw dygraph
dygraph(beans_xts, main = "Brinjal Price in Dambulla (Retail)") %>%
...
# Filter for a single series
beans_dambulla <- veg_filled %>%
  filter(Item == "Brinjal", Type == "Wholesale", Market == "Dambulla") %>%
  select(Date, Price) %>%
  arrange(Date)  # Ensure chronological order
...
# Draw dygraph
dygraph(beans_xts, main = "Brinjal Price in Dambulla (Wholesale)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
# Draw dygraph
dygraph(beans_xts, main = "Brinjal Price in Pettah (Retail)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
# Filter for a single series
beans_dambulla <- veg_filled %>%
  filter(Item == "Brinjal", Type == "Wholesale", Market == "Pettah") %>%
  select(Date, Price) %>%
  arrange(Date)  # Ensure chronological order
...
# Draw dygraph
dygraph(beans_xts, main = "Brinjal Price in Pettah (Wholesale)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
p3 <- veg_filled |>
  filter(Year > 2022) |>
  filter(Item == "Brinjal", Type == "Retail", Market == "Pettah") |>
  mutate(
    Week = isoweek(Date),
    Wday = factor(
      wday(Date, label = TRUE, abbr = TRUE, week_start = 1),
...
pim <- ggplot(BRP_inter, aes(x = Date)) +
  ...
  labs(
    y = "Price",
    title = "Brinjal Retail Price – Pettah Market",
    colour = "Interpolated"
  ) +
  theme_minimal() 
pim
