...
beans_dambulla <- veg_filled %>%
  filter(Item == "Big Onion (Imp)", Type == "Retail", Market == "Dambulla") %>%
  select(Date, Price) %>%
...
# Draw dygraph
dygraph(beans_xts, main = "Big Onion (Imp) Price in Dambulla (Retail)") %>%
...
# Filter for a single series
beans_dambulla <- veg_filled %>%
  filter(Item == "Big Onion (Imp)", Type == "Wholesale", Market == "Dambulla") %>%
  select(Date, Price) %>%
  arrange(Date)  # Ensure chronological order
...
# Draw dygraph
dygraph(beans_xts, main = "Big Onion (Imp) Price in Dambulla (Wholesale)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
# Draw dygraph
dygraph(beans_xts, main = "Big Onion (Imp) Price in Pettah (Retail)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
# Filter for a single series
beans_dambulla <- veg_filled %>%
  filter(Item == "Big Onion (Imp)", Type == "Wholesale", Market == "Pettah") %>%
  select(Date, Price) %>%
  arrange(Date)  # Ensure chronological order
...
# Draw dygraph
dygraph(beans_xts, main = "Big Onion (Imp) Price in Pettah (Wholesale)") %>%
  dyAxis("y", label = "Price (LKR)") %>%
  dyRangeSelector()  # Adds interactive date range
```
...
p3 <- veg_filled |>
  filter(Year > 2022) |>
  filter(Item == "Big Onion (Imp)", Type == "Retail", Market == "Pettah") |>
  mutate(
    Week = isoweek(Date),
    Wday = factor(
      wday(Date, label = TRUE, abbr = TRUE, week_start = 1),
